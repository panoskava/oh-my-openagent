...
匿名遥测默认开启，用于帮助提升安装和运行时的可靠性。它使用 PostHog，只记录哈希化的安装标识符，绝不会使用原始主机名。可以通过 `OMO_SEND_ANONYMOUS_TELEMETRY=0` 或 `OMO_DISABLE_POSTHOG=1` 禁用。详见 [隐私政策](docs/legal/privacy-policy.md) 和 [服务条款](docs/legal/terms-of-service.md)。
