...
匿名のテレメトリは、インストールとランタイムの信頼性向上のためにデフォルトで有効になっています。PostHog を使用し、生のホスト名ではなくハッシュ化されたインストール識別子のみを記録します。無効化するには `OMO_SEND_ANONYMOUS_TELEMETRY=0` または `OMO_DISABLE_POSTHOG=1` を設定してください。詳細は [プライバシーポリシー](docs/legal/privacy-policy.md) と [利用規約](docs/legal/terms-of-service.md) をご覧ください。
